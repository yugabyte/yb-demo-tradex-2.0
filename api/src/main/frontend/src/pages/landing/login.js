...
import { doLogin, doPassReset } from "../../services/authService";
...
export default function Login() {
  ...
  const handleLogin = (event) => {
    ...
    doLogin(signinRequestBody).then(
      (resp) => {
        if ( resp.status === 'SUCCESS'){
          window.localStorage.setItem('authToken', resp.token);
          window.localStorage.setItem('myPrefLocation', resp.preferredRegion);
          setPhase("success")
        }
        else {
  ...
  const handlePasswordReset = (event) => {
    ...
    if ( email !== '') {
      ...
      doPassReset(passwdChangeRequestBody).then(
        (resp) => {
          if ( resp.status ){         
            window.localStorage.setItem('authToken', resp.token);
            window.localStorage.setItem('myPrefLocation', resp.preferredRegion);
            setSuccess(true);
            setSuccessMsg('New password: ' + resp.password);
          }
          else {
            setError(resp.message);
            setSuccess(false);
      ...
      setError('Email cant be blank to reset password');
    }
        ...
        <div className={classes.landingHeader}>
              ...
              <Grid container spacing={2}>
                  ...
                  <Grid item xs={12} sx={{mb:6}}>
                    <Typography variant="h1" component="h2" sx={{fontWeight:600}}>Sign in with Password </Typography>
                  </Grid>

                  <Grid item xs={12}>
                    <TextField fullWidth type="email" label="Email" id="fullWidth" value={email}  onChange={(e) => setEmail(e.target.value)}/>
                  </Grid>

                  <Grid item xs={12}>
                      <TextField fullWidth type="password" label="Password" id="fullWidth" value={password} onChange={(e) => setPassword(e.target.value)} />
                  </Grid>
                  ...
                  <Grid item xs={12}>
                      ...
                      <FormControlLabel control={<Checkbox/>}label="Remember me"/>
                        ...
                        <Link onClick={handlePasswordReset} style={{marginRight:"30px",float:"right", lineHeight:"2.6rem", textDecoration:"none !important", fontWeight:"400", color:"#289BF6 !important"}}>
                          Reset Password
                        </Link> 
                    
                      <Button disabled={( email === '' || password === '')} className="css-uyw6s9-MuiButtonBase-root-MuiButton-root" variant="contained" color="success" onClick={handleLogin} sx={{width:270,borderRadius:3}}>
                        SIGN IN
                      </Button>
            ...
            <Grid item xs={12}>
              ...
              Don't have an account?
                ...
                <Link to={"/createnew"} sx={{textDecoration:"none"}}>&nbsp; Sign Up</Link>
            ...
            {error && <>
                <Typography component="p" variant="body1" textAlign="center" style={{color: 'red', fontWeight: 'bold'}} >
                   {error}
                </Typography>
              </>
            }
            ...
            {
              success && <>
                <Typography component="p" variant="body1" textAlign="center" style={{color: 'green', fontWeight: 'bold'}} >
                   {successMsg}
                </Typography>
              </>
            } 
