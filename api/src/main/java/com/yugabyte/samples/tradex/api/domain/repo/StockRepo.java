...
public class StockRepo {
    ...
    public List<TradeXStock> getAllActiveTradeXStocks(TradeXDataSourceType dbType) {
        ...
        return template.query(provider.getStockSQL(SqlQueries.StockSql.ALL_ACTIVE_STOCKS), new TradeXStockRowMapper());
    }
    ...
    public TradeXStock getStockSymbol(TradeXDataSourceType dbType, int symbolId) {
        NamedParameterJdbcTemplate template = resolver.resolve(dbType);
        try {
            return template.queryForObject(provider.getStockSQL(SqlQueries.StockSql.STOCK_BY_ID_SQL),
    ...
    public List<TradeXHistoricalQuote> getTradeXHistory(TradeXDataSourceType dbType, int symbolId, TradeXHistoryIntervalPeriod intervalPeriod) {
        NamedParameterJdbcTemplate template = resolver.resolve(dbType);
        try {
            return template.query(provider.getStockSQL(SqlQueries.StockSql.STOCK_DAY_HISTORY_BY_ID_SQL),
                    Map.of("symbolId", symbolId, "intervalPeriod", intervalPeriod.getValue()), new TradeXHistoricalQuoteRowMapper());
        } catch (EmptyResultDataAccessException e) {
            log.error("No data for symbol id:{}", symbolId);
            throw e;
        }
    ...
    public List<TradeXHistoricalQuote> getTradeXHistory(TradeXDataSourceType dbType, int symbolId, String fromDate, String toDate) {
        NamedParameterJdbcTemplate template = resolver.resolve(dbType);
        try {
            return template.query(provider.getStockSQL(SqlQueries.StockSql.STOCK_HISTORY_BY_ID_BETWEEN_SQL),
...
class TradeXStockRowMapper implements RowMapper<TradeXStock> {
    ...
    public TradeXStock mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        ts.setCreatedDate(rs.getTimestamp("created_date").toInstant());
        ...
        if (rs.getTimestamp("price_time") != null) {
            ts.setPriceTime(rs.getTimestamp("price_time").toInstant());
        }
        return ts;
    }
}
...
class TradeXHistoricalQuoteRowMapper implements RowMapper<TradeXHistoricalQuote> {
    ...
    public TradeXHistoricalQuote mapRow(ResultSet rs, int rowNum) throws SQLException {
        ...
        historicalQuote.setDate(rs.getTimestamp("price_time").toInstant());
